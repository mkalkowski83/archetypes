...
/**
 * Public API - commands for interacting with ProductFacade and ProductCatalog. Commands represent
 * write operations (mutations). All fields use simple types - no domain objects leak through the
 * API.
 */
public class ProductCommands {
    ...
    /**
     * Base interface for feature constraint configurations. Each implementation represents a
    ...
    public record AllowedValuesConfig(Set<String> allowedValues)
            implements FeatureConstraintConfig {
    ...
    public record DefineProductType(
        ...
        public DefineProductType {
            checkArgument(
                    productIdType != null && !productIdType.isBlank(),
                    "Product ID type must be defined");
            ...
            checkArgument(
                    description != null && !description.isBlank(), "Description must be defined");
            checkArgument(unit != null && !unit.isBlank(), "Unit must be defined");
            checkArgument(
                    trackingStrategy != null && !trackingStrategy.isBlank(),
                    "Tracking strategy must be defined");
        }
    }
    ...
    public record AddToOffer(
        ...
        public AddToOffer {
            checkArgument(
                    productTypeId != null && !productTypeId.isBlank(),
                    "Product type ID must be defined");
            checkArgument(
    ...
    public record DiscontinueProduct(
        ...
        public DiscontinueProduct {
            checkArgument(
                    catalogEntryId != null && !catalogEntryId.isBlank(),
                    "Catalog entry ID must be defined");
    ...
    public record DefinePackageType(
        ...
        public DefinePackageType {
            ...
            checkArgument(
                    ...
                    "Tracking strategy must be defined");
            checkArgument(
                    selectionRules != null && !selectionRules.isEmpty(),
                    "Selection rules must be defined");
        }
    }

    /**
     * Base interface for selection rule configurations. Each implementation represents a specific
     * type of rule.
     */
    ...
    /**
     * IsSubsetOf rule: select min to max products from a set. Example: Select 2-3 accessories from
    ...
    public record IsSubsetOfConfig(
        ...
        public IsSubsetOfConfig {
            checkArgument(
                    productIds != null && !productIds.isEmpty(), "Product IDs must not be empty");
            checkArgument(min >= 0, "Min must be >= 0");
            checkArgument(max >= min, "Max must be >= min");
        }
    }

    /** Single rule: exactly one product from the set. Shorthand for IsSubsetOf(set, 1, 1). */
    public record SingleConfig(Set<String> productIds // Product identifiers in the set
            ) implements SelectionRuleConfig {
        public SingleConfig {
            checkArgument(
                    productIds != null && !productIds.isEmpty(), "Product IDs must not be empty");
    ...
    /** Optional rule: zero or one product from the set. Shorthand for IsSubsetOf(set, 0, 1). */
    public record OptionalConfig(Set<String> productIds // Product identifiers in the set
            ) implements SelectionRuleConfig {
    ...
    /**
     * Required rule: at least one product from the set. Shorthand for IsSubsetOf(set, 1,
     * Integer.MAX_VALUE).
     */
    public record RequiredConfig(Set<String> productIds // Product identifiers in the set
            ) implements SelectionRuleConfig {
    ...
    public record IfThenRuleConfig(
            SelectionRuleConfig condition, Set<SelectionRuleConfig> thenRules)
            implements SelectionRuleConfig {
        public IfThenRuleConfig {
            checkNotNull(condition, "Condition must be defined");
            checkArgument(
                    thenRules != null && !thenRules.isEmpty(), "Then rules must not be empty");
        }
    }
    ...
    public record FeatureInstanceConfig(String featureName, String value) {
        public FeatureInstanceConfig {
            checkArgument(
                    featureName != null && !featureName.isBlank(), "Feature name must be defined");
    ...
    public record CreatePackageInstance(
        ...
        public CreatePackageInstance {
            checkArgument(
                    packageTypeId != null && !packageTypeId.isBlank(),
                    "Package type ID must be defined");
    ...
    public record SelectedInstanceConfig(
        ...
        public SelectedInstanceConfig {
            checkArgument(
                    instanceId != null && !instanceId.isBlank(), "Instance ID must be defined");

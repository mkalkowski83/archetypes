...
/**
 * PackageInstance represents a specific instance of a PackageType with concrete instances
 * delivered.
 *
 * <p>Examples: - PackageType: "Laptop Bundle" -> PackageInstance: delivered laptop SN-ABC, RAM
 * batch-001, SSD SN-XYZ - PackageType: "Banking Package" -> PackageInstance: delivered Gold card
 * SN-123, insurance policy POL-456
 *
 * <p>PackageInstance tracks: - Which PackageType was purchased - Concrete instances delivered
 * (ProductInstance or PackageInstance) - Optional serial number or batch for the package itself
 *
 * <p>The delivered instances are validated against the PackageType's structure to ensure all rules
 * are satisfied.
 */
class PackageInstance implements Instance {
    ...
    private static void validateTrackingRequirements(
            PackageType packageType, SerialNumber serialNumber, BatchId batchId) {
        checkArgument(
        ...
        if (strategy.isTrackedIndividually() && serialNumber == null) {
            throw new IllegalArgumentException(
                    "PackageType requires individual tracking (strategy: "
                            + strategy
        ...
        if (strategy.isTrackedByBatch() && batchId == null) {
            throw new IllegalArgumentException(
                    "PackageType requires batch tracking (strategy: "
                            + strategy
        ...
        if (strategy.requiresBothTrackingMethods() && (serialNumber == null || batchId == null)) {
            throw new IllegalArgumentException(
                    "PackageType requires both individual and batch tracking (strategy: "
                            + strategy
                            + ")");
        }
    }
    ...
    private static void validateSelection(
            PackageType packageType, List<SelectedInstance> selection) {

...
class PostingRulesScenarios {
    ...
    PostingRulesConfiguration postingRulesConfig =
            PostingRulesConfiguration.inMemory(
    ...
    void should_execute_posting_rule_when_entry_is_created() {
        ...
        // and commission rule: 2% of credit entries to receivables account goes to commission
        // tracking
        PostingRule commissionRule =
                PostingRuleBuilder.createRule("Commission tracking")
    ...
    void should_execute_posting_rules_in_priority_order_when_order_matters() {
        ...
        PostingRule taxRule =
                PostingRuleBuilder.createRule("Tax deduction")
                        .when(accountEquals(receivablesAccount).and(entryTypeEquals(CREDIT)))
        ...
        PostingRule bonusRule =
                PostingRuleBuilder.createRule("Bonus tracking")
                        .when(
                                accountEquals(receivablesAccount)
                                        .and(entryTypeEquals(CREDIT))
                                        .and(new MinimumBalanceCondition(pln(1000))))
    ...
    void should_save_and_find_posting_rule() {
        // given
        PostingRule rule =
                PostingRuleBuilder.createRule("Test rule")
    ...
    void should_update_existing_posting_rule() {
        ...
        PostingRule originalRule =
                PostingRuleBuilder.createRule("Original rule")
        ...
        ConfigurablePostingRule updatedRule =
                new ConfigurablePostingRule(
                        originalRule.id(),
                        "Updated rule",
    ...
    private AccountId generateOffBalanceAccount() {
        ...
        assertTrue(
                accountingFacade
                        .createAccount(
                                CreateAccount.generateOffBalanceAccount(
                                        accountId, randomStringWithPrefixOf("acc")))
                        .success());
        return accountId;
    }

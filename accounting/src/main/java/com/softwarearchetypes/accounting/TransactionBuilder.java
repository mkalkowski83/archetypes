...
public class TransactionBuilder {
    ...
    public TransactionBuilder withTransactionEntriesConstraint(
    ...
    public ReverseTransactionEntriesBuilder reverting(TransactionId refTransactionId) {
        Transaction refTransaction =
                transactionRepository
                        .find(refTransactionId)
                        .orElseThrow(
                                () ->
                                        new IllegalArgumentException(
                                                String.format(
                                                        "Transaction %s does not exist",
                                                        refTransactionId.toString())));
        return new ReverseTransactionEntriesBuilder(refTransaction);
    }
        ...
        Entry entry =
                entryRepository
                        ...
                        .orElseThrow(
                                () ->
                                        new IllegalArgumentException(
                                                String.format("Entry %s does not exist", entryId)));
    ...
    public class TransactionEntriesBuilder {
        ...
        public TransactionEntriesBuilder creditTo(AccountId accountId, Money amount) {
            AccountCredited entry =
                    new AccountCredited(
                            accountId, transactionId, amount, appliesAt, occurredAt, metadata);
            this.entries.add(entry);
            this.involvedAccountsIds.add(accountId);
            return this;
        }

        public TransactionEntriesBuilder creditTo(
                AccountId accountId, Money amount, Validity validity) {
            return creditTo(accountId, amount, validity, EntryAllocationFilter.NONE);
        }

        public TransactionEntriesBuilder creditTo(
                AccountId accountId, Money amount, EntryId appliedTo) {
            return creditTo(
                    accountId, amount, EntryAllocationFilterBuilder.manual(appliedTo).build());
        }

        public TransactionEntriesBuilder creditTo(
                AccountId accountId, Money amount, Validity validity, EntryId appliedTo) {
            return creditTo(
                    accountId,
                    amount,
                    validity,
                    EntryAllocationFilterBuilder.manual(appliedTo).build());
        }

        public TransactionEntriesBuilder creditTo(
                AccountId accountId, Money amount, EntryAllocationFilter filter) {
        ...
        public TransactionEntriesBuilder debitFrom(
                AccountId accountId, Money amount, Validity validity) {
        ...
        public Transaction build() {
            ...
            if (accountsInvolved.size() == involvedAccountsIds.size()) {
                Map<Entry, Account> entriesWithAccounts =
                        entries.stream()
                                .collect(
                                        toMap(
                                                it -> it,
                                                it -> accountsInvolved.get(it.accountId())));
                ...
                String missingAccountIds =
                        subtract(involvedAccountsIds, accountsInvolved.keySet()).stream()
                                .map(AccountId::uuid)
                                .map(UUID::toString)
                                .collect(joining(","));
                throw new IllegalArgumentException(
                        String.format("Accounts %s does not exist", missingAccountIds));
            }
        }
        ...
        private EntryId findEntryAllocation(EntryAllocationFilter filter) {
            ...
            Entry foundEntry =
                    entryAllocations
                            .findAllocationFor(filter)
                            .orElseThrow(
                                    () ->
                                            new IllegalArgumentException(
                                                    "No matching entry found for allocation"));
            ...
            if (!referencedEntry.validity().isValidAt(appliesAt)) {
                throw new IllegalArgumentException(
                        String.format(
                                ...
                                referencedEntry.id().toString(), appliesAt.toString()));
            }
        }
    }
    ...
    public class ReverseTransactionEntriesBuilder {
        ...
        ReverseTransactionEntriesBuilder(Transaction refTransaction) {
            ...
            refTransaction.entries().entrySet().stream()
     ...
     *
     * <p>Note: A scheduler service could be implemented to automatically find expired entries using
     * EntryRepository.findAllMatching() with validity.hasExpired() predicate, and then call this
     * builder to create compensation transactions via AccountingFacade.
     */
    public class ExpirationCompensationTransactionEntriesBuilder {
        ...
        ExpirationCompensationTransactionEntriesBuilder(Entry refEntry) {
            checkArgument(
                    refEntry.validity().hasExpired(clock.instant()),
                    String.format("Entry %s has not expired yet", refEntry.id()));
            this.refEntry = refEntry;
            this.account =
                    accountRepository
                            .find(refEntry.accountId())
                            .orElseThrow(
                                    () ->
                                            new IllegalArgumentException(
                                                    String.format(
        ...
        ExpirationCompensationTransactionEntriesBuilder withCompensationAccount(
                AccountId accountId) {
                    ...
                    accountRepository
                            ...
                            .orElseThrow(
                                    () ->
                                            new IllegalArgumentException(
                                                    String.format(
                                                            "Compensation account %s does not exist",
                                                            refEntry.accountId())));
            return this;
        }
        ...
        private Money calculateRemainingAmount() {
            Money referencingEntriesTotalAmount =
                    entryRepository.findEntriesReferencing(refEntry).stream()
                            .map(Entry::amount)

...
public final class Transaction {
    ...
    Transaction(
        ...
        // these two constraints are usually true for on-balance transactions, but does not have to
        // be true if off-balance
        // accounts are involved - these cases were moved to TransactionEntriesConstraint
        // implementation
        // checkArgument(entries != null && entries.size() >= 2, "Transaction must have at least 2
        // entries");
        // checkArgument(new HashSet<>(entries.values()).size() >= 2, "Transaction must involve at
        // least 2 accounts");
        checkArgument(
                transactionEntriesConstraint.test(entries),
                transactionEntriesConstraint.errorMessage());
        this.id = id;
        this.refId = refId;
        this.type = type;
...
interface TransactionEntriesConstraint extends Predicate<Map<Entry, Account>> {
    ...
    TransactionEntriesConstraint BALANCING_CONSTRAINT =
            new TransactionEntriesConstraint() {
                @Override
                ...
                public boolean test(Map<Entry, Account> entries) {
                    Money balance =
                            Optional.ofNullable(entries).orElse(Map.of()).entrySet().stream()
                                    .filter(
                                            entry ->
                                                    entry.getValue()
                                                            .type()
                                                            .isDoubleEntryBookingEnabled())
    ...
    TransactionEntriesConstraint MIN_2_ACCOUNTS_INVOLVED_CONSTRAINT =
            new TransactionEntriesConstraint() {
                ...
                @Override
                public boolean test(Map<Entry, Account> entries) {
                    return new HashSet<>(Optional.ofNullable(entries).orElse(Map.of()).values())
                                    .size()
                            >= 2;
                }
            };

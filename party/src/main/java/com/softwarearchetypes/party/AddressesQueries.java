...
public class AddressesQueries {
    ...
    public List<AddressView> findAllFor(PartyId partyId) {
        return repository.findFor(partyId).map(Addresses::asSet).orElse(java.util.Set.of()).stream()
                .map(AddressViewMapper::toView)
                .toList();
    }

    /**
     * Sample query using predicate on domain objects. NOTE: In production, replace with proper
     * Criteria pattern.
     */

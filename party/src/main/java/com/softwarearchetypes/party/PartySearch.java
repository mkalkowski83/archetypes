...
class PartySearch {
    ...
    /**
     * Sample query implementation using predicates on domain objects. NOTE: This is an example
     * implementation. In production, this should be replaced with: - Search Criteria objects
     * instead of Predicates - Direct database queries (e.g., SQL joins, graph database queries) -
     * Avoiding loading domain objects just for filtering
     *
     * <p>This is the place where we could apply graph database queries or advanced search engines.
     */
    List<PartyView> findMatching(Search search) {
        ...
        if (search.partyRelationshipPredicate != null) {
            if (!result.isEmpty()) {
                ...
                List<PartyId> toPartyIds =
                        partyRelationshipsQueries
                                .findMatching(
                                        it ->
                                                foundIds.contains(it.from().partyId())
                                                        && search.partyRelationshipPredicate.test(
                                                                it))
                                .stream()
    ...
    /**
     * Search criteria using predicates on domain objects. NOTE: In production, replace with proper
     * Criteria/Specification pattern.
     */
    record Search(
            Predicate<Party> partyPredicate,

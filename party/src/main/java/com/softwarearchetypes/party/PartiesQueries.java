...
public class PartiesQueries {
    ...
    public Optional<PartyView> findBy(PartyId partyId) {
        return partyRepository.findBy(partyId).map(PartyViewMapper::toView);
    }
    ...
    public Optional<PartyView> findOneBy(RegisteredIdentifier registeredIdentifier) {
        ...
        if (partiesMatching.size() > 1) {
            throw new IllegalStateException(
                    format(
                            "There are more than one parties with the same registered identifier of %s",
                            registeredIdentifier));
        }
        return partiesMatching.stream().findFirst().map(PartyViewMapper::toView);
    }

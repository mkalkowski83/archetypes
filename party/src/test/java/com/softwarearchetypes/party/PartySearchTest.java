...
class PartySearchTest {
    ...
    private final InMemoryPartyRelationshipRepository partyRelationshipRepository =
    ...
    private final PartyRelationshipTestSupport partyRelationshipTestSupport =
            new PartyRelationshipTestSupport(facade);

    // address
    private final InMemoryAddressesRepository addressesRepository =
            new InMemoryAddressesRepository();
    private final AddressesFacade addressesFacade =
            new AddressesFacade(addressesRepository, eventPublisher);
    private final AddressesTestSupport addressesTestSupport =
            new AddressesTestSupport(addressesFacade);
    private final AddressesQueries addressesQueries = new AddressesQueries(addressesRepository);

    // object under test
    private final PartySearch partySearch =
            new PartySearch(partiesQueries, partyRelationshipsQueries, addressesQueries);
    ...
    void shouldFindAnEmployeeOfAProviderThatWorksInWarsaw() {
        ...
        Party firstProvider =
                testSupport.thereIs(
                        someCompany()
                                .with(OrganizationName.of("FIRST Inc."))
                                .withRandomPartyId()
                                .with(PROVIDER)
                                .build());

        Party romanTheContractorFromWarsaw =
                testSupport.thereIs(somePerson().with(nameOf("Roman")).withRandomPartyId().build());
        addressesTestSupport.thereIsAddressOf(
                someGeoAddressFor(romanTheContractorFromWarsaw.id(), "WARSAW"));
        partyRelationshipTestSupport.thereIsARelationBetween(
                firstProvider, ORDERER, romanTheContractorFromWarsaw, CONTRACTOR, CONTRACT);

        Party peterTheEmployeeFromKrakow =
                testSupport.thereIs(somePerson().with(nameOf("Peter")).withRandomPartyId().build());
        addressesTestSupport.thereIsAddressOf(
                someGeoAddressFor(peterTheEmployeeFromKrakow.id(), "KRAKOW"));
        partyRelationshipTestSupport.thereIsARelationBetween(
                firstProvider, EMPLOYER, peterTheEmployeeFromKrakow, EMPLOYEE, EMPLOYMENT);

        // and - second provider graph
        Party secondProvider =
                testSupport.thereIs(
                        someCompany()
                                .with(OrganizationName.of("SECOND Inc."))
                                .withRandomPartyId()
                                .with(PROVIDER)
                                .build());

        Party adamTheEmployeeFromWarsaw =
                testSupport.thereIs(somePerson().with(nameOf("Adam")).withRandomPartyId().build());
        addressesTestSupport.thereIsAddressOf(
                someGeoAddressFor(adamTheEmployeeFromWarsaw.id(), "WARSAW"));
        partyRelationshipTestSupport.thereIsARelationBetween(
                secondProvider, EMPLOYER, adamTheEmployeeFromWarsaw, EMPLOYEE, EMPLOYMENT);

        Party chrisTheEmployeeFromLublin =
                testSupport.thereIs(somePerson().with(nameOf("Chris")).withRandomPartyId().build());
        addressesTestSupport.thereIsAddressOf(
                someGeoAddressFor(chrisTheEmployeeFromLublin.id(), "LUBLIN"));
        partyRelationshipTestSupport.thereIsARelationBetween(
                secondProvider, EMPLOYER, chrisTheEmployeeFromLublin, EMPLOYEE, EMPLOYMENT);

        Party mikeTheEmployeeBdg =
                testSupport.thereIs(somePerson().with(nameOf("Mike")).withRandomPartyId().build());
        addressesTestSupport.thereIsAddressOf(someGeoAddressFor(mikeTheEmployeeBdg.id(), "BDG"));
        partyRelationshipTestSupport.thereIsARelationBetween(
                secondProvider, EMPLOYER, mikeTheEmployeeBdg, EMPLOYEE, EMPLOYMENT);

        // and - partner graph
        Party partner =
                testSupport.thereIs(
                        someCompany()
                                .with(OrganizationName.of("PARTNER Inc."))
                                .withRandomPartyId()
                                .with(PARTNER)
                                .build());

        Party johnTheEmployeeFromWarsaw =
                testSupport.thereIs(somePerson().with(nameOf("John")).withRandomPartyId().build());
        addressesTestSupport.thereIsAddressOf(
                someGeoAddressFor(johnTheEmployeeFromWarsaw.id(), "WARSAW"));
        partyRelationshipTestSupport.thereIsARelationBetween(
                partner, EMPLOYER, johnTheEmployeeFromWarsaw, EMPLOYEE, EMPLOYMENT);

        // when searching for an employee of our provider who lives in Warsaw
        PartySearch.Search search =
                new PartySearch.Search(
                        party -> party.roles().contains(PROVIDER),
                        rel -> rel.name().equals(EMPLOYMENT),
                        address ->
                                address instanceof GeoAddress
                                        && ((GeoAddress.GeoAddressDetails) address.addressDetails())
                                                .city()
                                                .equals("WARSAW"));
